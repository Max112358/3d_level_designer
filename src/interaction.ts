...
const ALL_FACES: SurfaceFace[] = ["floor", "ceiling", ...FACES];
...
export function useSceneInteraction() {
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (dist > 3) isDraggingRef.current = true;
      ...
      const raycaster = getRaycaster(e, gl.domElement, camera);
      ...
      Object.keys(level.cells).forEach((key) => {
        ...
        ALL_FACES.forEach((face) => {
          ...
          const matId = getCellFaceMaterial(cell, face);
          if (matId && matId !== "none") {
      ...
      // 2. Fallback virtual plane raycast
      if (!best) {
        if (category === "ceiling") {
          ...
          if (raycaster.ray.intersectPlane(plane, p)) {
            best = {
              key: cellKey(
                Math.floor(p.x / CELL_SIZE),
                layerY,
                Math.floor(p.z / CELL_SIZE),
              ),
              face: "ceiling",
              point: p,
          ...
          if (raycaster.ray.intersectPlane(plane, p)) {
            ...
            const chosenFace: Direction =
              Math.abs(localX) > Math.abs(localZ)
                ? localX > 0
                  ? "east"
                  : "west"
                : localZ > 0
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (tool === "paint" && activeAssetId) {
        ...
        if (cell) {
          const id = getCellFaceMaterial(cell, hover.face);
          if (id && id !== "none") {
            setActiveAssetId(id);
